...
public class Member
{
    ...
    /**
     * Returns the server-side relative path for this member
     *
     * @return
     */    
     ...
     * @throws APIException
     */
    public boolean checkout( APISession api, String version, String cpid)
        throws APIException
    {
        ...
        coCMD.addOption( new Option( "lock" ) );
        coCMD.addOption( new Option( "moveLock" ) );
        coCMD.addOption( new Option( "update" ) );
        coCMD.addOption( new Option( "project", projectConfigPath ) );
        coCMD.addOption( new Option( "targetFile", targetFile.getAbsolutePath()));
        coCMD.addOption( new Option( restoreTimestamp ) );
        coCMD.addOption( new Option( "lineTerminator", lineTerminator ) );
        
        if(version != null && !version.equals("")){
        	coCMD.addOption( new Option( "revision", version) );
        }else{
        	coCMD.addOption( new Option( "revision", memberRev ) );
        }
        
        coCMD.addOption( new Option("cpid", cpid));
        // Add the member selection
        coCMD.addSelection( memberID );
        logger.info("*** Before checkout"+targetFile.getAbsolutePath()+":"+version+":");
        // Execute the checkout command
        Response res = api.runCommand( coCMD );
        logger.info("*** After checkout");
